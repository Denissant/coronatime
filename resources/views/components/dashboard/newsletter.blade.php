<section class="flex flex-col items-center bg-gradient-linear-green py-10 rounded-2xl mt-[7vh] sm:mt-[13vh]">
    <h4 class="font-black text-2xl">{{ __('dashboard.get_notified') }}</h4>
    <p class="font-normal mt-4">{{ __('dashboard.get') }} <span class="font-bold">{{ __('dashboard.personalised') }}</span> {{ __('dashboard.notifications_via_email') }}</p>
    <label for="email" class="sr-only">Newsletter email</label>
    <form class="relative w-11/12 lg:w-1/3" method="POST">
        @csrf
        <input id="email" name="email" type="email" placeholder="{{ __('auth.enter_email') }}"
