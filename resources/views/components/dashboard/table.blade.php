...
<table class="block overflow-auto absolute left-0 max-h-[60vh] mt-6 sm:mt-10 sm:static sm:shadow-lg sm:rounded-lg min-w-full border-collapse">
    <thead class="sticky top-0 bg-[#F6F6F7] sm:rounded-lg flex">
        <tr class="h-14 sm:rounded-lg flex items-center w-full">
            <x-dashboard.table-column name="name" class="sm:rounded-tl-lg">{{ __('dashboard.location') }}</x-dashboard.table-column>
            <x-dashboard.table-column name="statistics.confirmed">{{ __('dashboard.new_cases') }}</x-dashboard.table-column>
            <x-dashboard.table-column name="statistics.deaths">{{ __('dashboard.deaths') }}</x-dashboard.table-column>
            <x-dashboard.table-column name="statistics.recovered" class="sm:rounded-tr-lg">{{ __('dashboard.recovered') }}</x-dashboard.table-column>
        </tr>
    </thead>
    <tbody class="divide-y divide-[#F6F6F7] flex flex-col w-full">
        <x-dashboard.table-row :location="__('dashboard.worldwide')" :confirmed="$worldwideStats['confirmed']"
